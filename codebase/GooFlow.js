...
//初始化区域图的对象
	...
	if(GooFlow.prototype.color.main){
		this.$bgDiv.append('<css>.GooFlow_tool_btndown{background:'+GooFlow.prototype.color.main+'}</css>');
	}
	...
	if(property.haveHead){
		tmp="<div class='GooFlow_head' "+(GooFlow.prototype.color.main? "css='border-bottom-color:"+GooFlow.prototype.color.main+"'" : "")
		+">";
		if(property.headLabel){
      tmp+="<label title='"+(property.initLabelText||"newFlow_1")+"' "
        +(GooFlow.prototype.color.main? "css='background:"+GooFlow.prototype.color.main+"'" : "")+">"+(property.initLabelText||"newFlow_1")+"</label>";
		}
	...
	if(property.haveTool){
		this.$bgDiv.append("<div class='GooFlow_tool'"+(property.haveHead? "":" css='margin-top:4px'")+"><div css='height:"+(height-headHeight-(property.haveHead? 5:8))+"px' class='GooFlow_tool_div'></div></div>");
		this.$tool=this.$bgDiv.find(".GooFlow_tool div");
		//未加代码：加入绘图工具按钮
		this.$tool.append("<div css='margin-bottom:5px'><span/><span/><span/><span/></div>"
	...
	this.$bgDiv.append("<div class='GooFlow_work' css='width:"+(width)+"px;height:"+(height)+"px;"+(property.haveHead? "":"margin-top:3px")+"'></div>");
	this.$workArea=$("<div class='GooFlow_work_inner' css='width:"+width+"px;height:"+height+"px'></div>")
	...
	if(this.$editable){
	  ...
	  this.$lineMove=$('<div class="GooFlow_linemove" css="display:none"></div>');//操作折线时的移动框
	  ...
	  this.$lineOper=$("<div class='GooFlow_line_oper' css='display:none'><i class='b_l1'></i><i class='b_l2'></i><i class='b_l3'></i><i class='b_x'></i></div>");//选定线时显示的操作框
	  ...
	  //新增移动线两个端点至新的结点功能移动功能，这里要提供移动用的DOM
		this.$mpFrom=$("<div class='GooFlow_line_mp' css='display:none'></div>");
		this.$mpTo=$("<div class='GooFlow_line_mp' css='display:none'></div>");
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		if(GooFlow.prototype.useSVG!=""){
			...
			this.$workArea.prepend("<div class='GooFlow_work_vml' css='position:relative;width:"+width+"px;height:"+height+"px'></div>");
	...
	initGroup:function(width,height){
		this.$group=$("<div class='GooFlow_work_group' css='width:"+width+"px;height:"+height+"px'></div>");//存放背景区域的容器
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round")<0){
			...
			this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+"' id='"+id+"' css='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' css='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td><td>"+json.name+"</td></tr></table><div css='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
		}
		else{
			json.width=26;json.height=26;
			this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+"' id='"+id+"' css='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td></tr></table><div  css='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
		}
...
///////////以下为有关画线的方法
	...
	addArea:function(id,json){
		...
		this.$areaDom[id]=$("<div id='"+id+"' class='GooFlow_area area_"+json.color+"' css='top:"+json.top+"px;left:"+json.left+"px'><div class='bg' css='width:"+(json.width)+"px;height:"+(json.height)+"px'></div>"
